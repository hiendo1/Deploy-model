...
MODEL_PATH = os.path.join(BASE_DIR, 'football_model_v2.joblib')
    ...
    try:
        ...
        return jsonify({
            ...
            'predictions': {
                ...
                'score': {
                    'home': round(float(home_goals), 1),
                    'away': round(float(away_goals), 1),
                    'total': round(float(home_goals + away_goals), 1)
                }
            },
            'metadata': {
